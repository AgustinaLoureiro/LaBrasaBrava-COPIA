...
`src/app/paginas/`. Los cambios se unen a `main` una vez por semana, antes de cada revisión de
...
Además, cada uno necesita completar `src/app/nucleo/configuracion.ts` con la dirección y la clave
...
1. **Cada uno toca solamente su carpeta** dentro de `src/app/paginas/`. En la carpeta de otro no
   mete mano nadie.
...
El reparto completo, punto por punto, está en restaurante/REPARTO-DE-TAREAS.md
...
1. Cada uno toca solamente su carpeta dentro de src/app/paginas/.
   En la carpeta del otro no mete mano nadie.

2. Hay cuatro archivos que usamos los cuatro: app.routes.ts,
   nucleo/marca.ts, theme/variables.scss y supabase/esquema.sql.
