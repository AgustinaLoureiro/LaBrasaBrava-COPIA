...
> del proyecto: ante cualquier duda sobre qué pide la cátedra, se consulta acá.
>
...
> El código de la aplicación vive en `TFI/LaBrasaBrava/`.

| Tema | Decisión | Fecha |
|---|---|---|
| Framework móvil | **Ionic 9 + Angular 22** (componentes standalone, sin NgRx) | 28-08-2026 |
| Animación de carga | La del manual de marca, **escrita en Angular con animaciones de CSS** (`componentes/logo-cargando`), porque el boceto original no se puede usar en Ionic | 04-09-2026 |
| Empaquetado nativo | **Capacitor 8** | 28-08-2026 |
| Backend / base de datos | **Supabase** (PostgreSQL + Auth + Storage + Realtime) | 28-08-2026 |
| Autenticación | **Provisoria**: validación contra las tablas `empleados` y `clientes` de la base del grupo, con la contraseña en texto plano. La migración a **Supabase Auth** está escrita en `LaBrasaBrava/supabase/esquema.sql` y solo toca `sesion.ts`. Login social en 2ª fecha (punto 23) | 04-09-2026 |
...
| Flujo de trabajo con Git | Una rama por integrante, `main` siempre compilando, unión semanal por Pull Request (ver `LaBrasaBrava/docs/FLUJO-DE-TRABAJO.md`) | 29-08-2026 |
| Integrantes y roles | *pendiente* (marcador en `src/app/nucleo/marca.ts`) | |
| Líder del grupo | *pendiente* | |

### 15.1 Convenciones de código
- **Una sola aplicación**, dentro de `LaBrasaBrava/`. No se crean carpetas de proyecto
  paralelas: los cuatro integrantes trabajan sobre el mismo `src/`.
- **Todo en español**: carpetas, clases, métodos, variables y comentarios.
- Carpetas: `nucleo/` (marca, diseño, configuración, `modelos/`, `servicios/` y `guardas/`),
  `componentes/` (componentes reutilizables) y `pages/` (una carpeta por pantalla). Quedan
  además `services/` y `home/`, de la primera versión del proyecto, todavía en uso.
...
- **Usuarios de prueba dados de alta** en la base: los siete perfiles que exige el enunciado,
  más un cliente pendiente y uno rechazado (`LaBrasaBrava/supabase/usuarios-de-prueba.mjs`).
- Pantalla principal con cierre de sesión que verifica el borrado de credenciales.
- Herramienta de revisión visual (`LaBrasaBrava/revision-visual.mjs`): saca las capturas del
...
- Repositorio privado creado con los seis docentes como colaboradores, y flujo de ramas acordado
  (`LaBrasaBrava/docs/FLUJO-DE-TRABAJO.md` y `LaBrasaBrava/docs/REPARTO-DE-TAREAS.md`).
...
- Migrar la autenticación a Supabase Auth con `LaBrasaBrava/supabase/esquema.sql`: hoy las
