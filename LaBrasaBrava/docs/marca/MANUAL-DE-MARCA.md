...
`LaBrasaBrava/src/assets/marca/`. No se dibuja a mano ni se reemplaza por un vector propio.


### Animación de carga

La animación del logo para las pantallas de espera está **escrita en Angular con animaciones
de CSS**, en `LaBrasaBrava/src/app/componentes/logo-cargando/`. Reconstruye las tres etapas de
la variante **Loader Pro** que definía el manual:

1. **Encendido**: la llama se revela de abajo hacia arriba.
2. **Aparición del nombre**: «Brasa Brava» sube y se asienta.
3. **Ciclo de brasas**: la llama respira y titila, las brasas suben y el anillo de progreso
   gira. El ciclo empalma solo y se repite mientras dure la espera.

Acepta `orange`, `carbon` o `crema` como parámetro de color, y se usa en dos lugares:

| Dónde | Cómo |
|---|---|
| Indicador de espera (`componentes/spinner-logo`) | Isotipo en crema sobre naranja brasa, a pantalla completa, con el nombre y el anillo de progreso. |
| Pantalla de presentación | Isotipo a color dentro del medallón, sin nombre ni anillo, porque la pantalla ya los tiene. |

**El fondo lo pone quien usa el componente**, nunca el componente mismo. Así se evita el fondo
casi negro (`#1A1512`) con el que venían las variantes del boceto original, que el enunciado y
el manual prohíben: la espera queda como pide el manual, isotipo en crema sobre naranja brasa.
...
con el texto. Viven en `LaBrasaBrava/src/app/nucleo/diseno.ts`:
...
| Qué | Archivo |
|---|---|
| Paleta y escala tipográfica como variables CSS | `LaBrasaBrava/src/theme/variables.scss` |
| Tipografías, clases de texto y componentes base | `LaBrasaBrava/src/global.scss` |
| Paleta y colores por perfil desde TypeScript | `LaBrasaBrava/src/app/nucleo/diseno.ts` |
| Logo en sus tres variantes de color | `LaBrasaBrava/src/assets/marca/` |
| Componente que elige la variante del logo | `LaBrasaBrava/src/app/componentes/logo-marca/logo-marca.component.ts` |
| Animación de carga del logo | `LaBrasaBrava/src/app/componentes/logo-cargando/logo-cargando.component.ts` |
| Indicador de espera a pantalla completa | `LaBrasaBrava/src/app/componentes/spinner-logo/spinner-logo.component.ts` |
| Datos del restaurante y del grupo | `LaBrasaBrava/src/app/nucleo/marca.ts` |
